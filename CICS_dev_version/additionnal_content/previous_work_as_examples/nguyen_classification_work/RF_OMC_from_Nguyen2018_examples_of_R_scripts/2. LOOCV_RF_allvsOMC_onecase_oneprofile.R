# we are here testing RF-all vs Rf-OMC on one case of classification data (BRCA-binimetinib-GEX) (test codename : RF1case)
...
# for (cancertype in c("BRCA","CRC")){ #RF1case
for (cancertype in c("BRCA")){ #RF1case
    ...
    # for (i in 1:ntreatments){
      ...
      if(file.exists(treatment_response_file)){ #blocked as noloop4
        ...
        if (sum(data[,1] %in% c("Sen")) >= 5 & sum(data[,1] == "Res") >= 5){  # deprecation1 applied
          ...
          # loop on the seed values to have more resuslts on prediction to draw curves (10 seeds with the zero being 5678 and the rest well you know it...)
          # n = 1
          ...
          for (n in 1:length(seeds)){
            ...
            for(j in 1:nrow(data)){
              ...
              # sens = length(which(train.output %in% c("CR","PR","SD"))) # deprecation1 applied
              sens = length(which(train.output %in% c("Sen"))) # deprecation1 applied
              # res = length(which(train.output == "PD")) # deprecation1 applied
              res = length(which(train.output == "Res")) # deprecation1 applied
              ...
              M3 = foreach(l = 1:nrow(train.data),.packages=c('randomForest','caret'),.combine = 'rbind', .inorder = TRUE) %dopar%{
                ...
                # sens = length(which(train.output.in %in% c("CR","PR","SD"))) # deprecation1 applied
                sens = length(which(train.output.in %in% c("Sen"))) # deprecation1 applied
                # res = length(which(train.output.in == "PD")) # deprecation1 applied
                res = length(which(train.output.in == "Res")) # deprecation1 applied
...
# close(fileConn)
# fileConn<-file("output.txt")
# writeLines(c("Hello1","World1"), fileConn)
# close(fileConn)
# cat("Hello",file="..outfile.txt",sep="\n")
# cat("Analysis duration is",file="outfile.txt",append=TRUE)
# cat(paste("Analysis of ",cancertype," with profile ",feat.type, " has taken ",time.taken, "\n",sep = ""))
